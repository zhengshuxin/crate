...
public class SetAnalyzerTest extends BaseAnalyzerTest {
    ...
    @Test
    public void testDynamicObjectValue() throws Exception {
        SetAnalysis analysis = (SetAnalysis) analyze("SET GLOBAL PERSISTENT cluster.routing.allocation.include = {tag='value1'}");
        assertThat(analysis.settings().toDelimitedString(','), is("cluster.routing.allocation.include.tag=value1,"));

        analysis = (SetAnalysis) analyze("SET GLOBAL PERSISTENT cluster.routing.allocation.include.tag = 'value1'");
        assertThat(analysis.settings().toDelimitedString(','), is("cluster.routing.allocation.include.tag=value1,"));
    }

    @Test
    public void testDynamicObjectValueInvalidNested() throws Exception {
        expectedException.expect(IllegalArgumentException.class);
        expectedException.expectMessage("setting 'cluster.routing.allocation.include.tag.foo' not supported");
        analyze("SET GLOBAL PERSISTENT cluster.routing.allocation.include = {tag={foo='value1'}}");
    }
    ...
    @Test
    public void testResetDynamic() throws Exception {
        SetAnalysis analysis = (SetAnalysis) analyze("SET GLOBAL PERSISTENT cluster.routing.allocation.include = {tag='value1'}");
        assertThat(analysis.settings().toDelimitedString(','), is("cluster.routing.allocation.include.tag=value1,"));

        analysis = (SetAnalysis) analyze("RESET GLOBAL cluster.routing.allocation.include");
        assertThat(analysis.isReset(), is(true));
        assertThat(analysis.settingsToRemove(), containsInAnyOrder("cluster.routing.allocation.include.tag"));
    }
}
