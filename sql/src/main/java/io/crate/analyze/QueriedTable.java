...
public class QueriedTable implements QueriedRelation {
    ...
    private static void replaceFields(QuerySpec querySpec,
        ...
        if (where != null && where.hasQuery()) {
            querySpec.where(new WhereClause(replaceFields(where.query(), fieldReplacingCtx), where.docKeys().orNull(), where.partitions()));
        }
    ...
    private static void splitWhereClause(TableRelation tableRelation,
        ...
        if (splitQueries.relationQuery() != null) {
            splitQuerySpec.where(new WhereClause(splitQueries.relationQuery(), where.docKeys().orNull(), where.partitions()));
            querySpec.where(new WhereClause(splitQueries.remainingQuery(), where.docKeys().orNull(), where.partitions()));
