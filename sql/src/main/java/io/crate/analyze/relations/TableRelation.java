...
public class TableRelation implements AnalyzedRelation, FieldResolver {
    ...
    public WhereClause processEqualities(WhereClause whereClause, EvaluatingNormalizer normalizer) {
        normalizer = normalizer.ensureInPlace(false);
        EqualityExtractor ee = new EqualityExtractor(normalizer);

        //ReferenceIdent clusteredBy = tableInfo.getReferenceInfo(tableInfo.clusteredBy()).ident();

        Set<Literal> clusteredBy = null;

        if (tableInfo.hasAutoGeneratedPrimaryKey()) {
            if (tableInfo.clusteredBy() != null){
                clusteredBy = getClusteredByLiterals(whereClause, ee);
            }


        } else {
            // check primary keys
            List<ReferenceIdent> pkCols = Lists.transform(tableInfo().primaryKey(), new com.google.common.base.Function<ColumnIdent, ReferenceIdent>() {
                @Nullable
                @Override
                public ReferenceIdent apply(@Nullable ColumnIdent input) {
                    return tableInfo.getReferenceInfo(input).ident();
                }
            });
            List<List<Symbol>> pkValues = ee.extract(pkCols, whereClause.query());
            if (pkValues != null) {
                int clusterdIdx = -1;
                List<Id> primaryKeys = new ArrayList<>(pkValues.size());
                if (tableInfo.clusteredBy() != null){
                ...
                for (List<Symbol> row : pkValues) {
                    Id id = new Id(tableInfo.primaryKey(),
                            Lists.transform(row, BytesRefValueSymbolVisitor.PROCESS_FUNCTION),
                            tableInfo.clusteredBy());
                    primaryKeys.add(id);
                    if (clusterdIdx>=0) {
                        clusteredBy.add((Literal) row.get(clusterdIdx));
                    }
                }
                whereClause.primaryKeys(primaryKeys);
                if (clusterdIdx>=0){
    ...
    @Nullable
    private Set<Literal> getClusteredByLiterals(WhereClause whereClause, EqualityExtractor ee) {
        if (tableInfo.clusteredBy() != null) {
            List<List<Symbol>> clusteredValues = ee.extract(
                    ImmutableList.of(tableInfo.getReferenceInfo(tableInfo.clusteredBy()).ident()),
                    whereClause.query());
            if (clusteredValues != null) {
                Set<Literal> clusteredBy = new HashSet<>(clusteredValues.size());
                for (List<Symbol> row : clusteredValues) {
                    clusteredBy.add((Literal) row.get(0));
                }
                return clusteredBy;
            }
        }
