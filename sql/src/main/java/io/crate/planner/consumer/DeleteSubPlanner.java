...
public class DeleteSubPlanner {
    ...
    private Plan deleteByQueryPlan(TableInfo tableInfo, WhereClause whereClause, Planner.Context context) {
        if (!whereClause.hasQuery() && !whereClause.partitions().isEmpty()) {
            List<PlanNode> planNodes = new ArrayList<>();
            for (String partition : whereClause.partitions()) {
                planNodes.add(new ESDeleteIndexNode(partition));
            }
            return new IterablePlan(planNodes);
        }
        Routing routing = tableInfo.getRouting(whereClause, null);
        context.allocateJobSearchContextIds(routing);
        DeleteByQueryNode deleteByQueryNode = new DeleteByQueryNode(context.nextExecutionNodeId(), routing, whereClause, null);
